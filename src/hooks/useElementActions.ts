...
import { createBox, createShelf, createBoard, createSzafkaDolna } from '../factories';
...
export function useElementActions({
  ...
  const handleAdd = useCallback((type: 'cabinet' | 'shelf' | 'board' | 'boxkuchenny' | 'szafkadolna60' | 'szafkadolna40' | 'szafkadolna30') => {
    const raw = type === 'shelf' ? createShelf() : type === 'board' ? createBoard() : type === 'szafkadolna60' ? createSzafkaDolna(0.6) : type === 'szafkadolna40' ? createSzafkaDolna(0.4) : type === 'szafkadolna30' ? createSzafkaDolna(0.3) : createBox();
    const bw = boardSizeRef.current.width / 1000;
